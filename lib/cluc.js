...
var Cluc = (function(){
  ...
  Cluc.prototype.when = function(thisIsTrue, fn){
    ...
    if(!!thisIsTrue && fn ){
      ...
      that.then(function(nextFN){
        var fakeLine = new Cluc();
        fn(fakeLine);
        var nLine = new Cluc();
        fakeLine.cmds.forEach(function(cmd){
          if(cmd.cmd){
            var cmdStr = that.processTemplate(cmd.cmd+'', that.savedValues);
            nLine.skipped('', cmdStr);
          }
        });
        nextFN(nLine);
      });
      ...
      function _next(context){
        ...
        if(context){
          ...
          if(execType.match(/(stream|tail)/)){
            ...
            cmd.fn.apply(context, [function waitFn(newLine){
              that.savedValues = _.extend(that.savedValues, context.savedValues);
              if(newLine) that.prepend(newLine);
              _next();
            }]);
