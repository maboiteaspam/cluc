...
var Cluc = (function(){
  ...
  /**
   * Skip next operation is skipNext is true
   *
   * @param skipNext bool
   * @param fn callback
   * @returns {Cluc}
   */
  Cluc.prototype.skip = function(skipNext, fn){
    ...
    if(this.cmds.length){
      ...
      function _next(context){
        ...
        if(context){
          ...
          }else if(execType=='subtitle'){
            var subtitle = processTemplate(cmd.d, that.savedValues);
            log.subtitle.apply(context, subtitle);
            that.savedValues = _.extend(that.savedValues, context.savedValues);
            _next();
