...
var Cluc = (function(){
   ...
   * @returns {Cluc}
   */
  Cluc.prototype.textedit = function(msg, fn){
    var unit = {fn: fn, t:'textedit', d:msg };
    this.pushCmd(unit);
    return this;
  };
  /**
      ...
      function _next(context){
        ...
        if(context){
          ...
          if(execType.match(/(stream|tail)/)){
            ...
            log.subtitle(cmd.d);
            temp.mkdir('cluc', function(err, dirPath) {
              var inputPath = path.join(dirPath, Date.now() + '.' + execType);
