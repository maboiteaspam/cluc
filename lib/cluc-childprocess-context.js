...
var ClucContext = (function(){
  ...
  ClucContext.prototype.init = function(cmdStr, savedValues){
    if(!this.hasRunOnce){
      this.canDo = 1;
      this.hasRunOnce = false;
    }
    this.cmd = cmdStr  || null;
    this.stdin = null;
    this.rules = [];
    this.failedRules = [];
  ...
  ClucContext.prototype.executeRules = function(error, stdout, stderr, stdin){
    ...
    try{
      if(this.cmd.fn) this.cmd.fn.call(this, error, stdout, stderr, stdin);
      // note that stdin is stdin of the underlying shell proces, not the stdin of the command,
      // so it won t work well if the command does not forward stdin.
    }catch(ex){
